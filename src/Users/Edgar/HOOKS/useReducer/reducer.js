...
function reducer(state = initialState, action) {
  switch (action.type) {
    case "change-title": return changetitle(state, action)
    case 'deluser': return deluser(state, action.payload.id)
...
const deluser = (state, id) => {
  console.log(1);
  const result = state.filter(e => e.id !== id)
  return result
}
